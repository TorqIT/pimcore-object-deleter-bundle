...
class ObjectDeleterBundleInstaller extends SettingsStoreAwareInstaller
{
    public function install()
    {
        ...
        BEGIN	
            ...
            INSERT INTO delete_obj(id)
                SELECT 
                    id
                FROM
                    objects
                WHERE
                    classId = classId COLLATE utf8mb4_general_ci
                    AND
                    path LIKE CONCAT(rootPath, '%');
                    
            -- return all objects being deleted
            SELECT id FROM delete_obj;
            ...
            FROM 
             ...
             JOIN 
                 delete_obj
             ON
                delete_obj.id = dependencies.sourceid
                AND
            ...
            JOIN 
                delete_obj
            ON
                delete_obj.id = dependencies.targetid
                AND
            ...
            JOIN
                delete_obj
            ON
                element_workflow_state.cid = delete_obj.id
                AND 
            ...
            JOIN
                delete_obj
            ON
                gridconfig_favourites.objectId = delete_obj.id;
            ...
            IF EXISTS (SELECT * FROM information_schema.tables WHERE TABLE_SCHEMA = database() AND table_name LIKE 'object_metadata_%') THEN
                SELECT @obj_meta := CONCAT('DELETE ', table_name, ' FROM ', table_name, ' JOIN delete_obj ON ', table_name, '.id = delete_obj.id;') FROM information_schema.tables WHERE TABLE_SCHEMA = database() AND table_name LIKE 'object_metadata_%';
            ...
            IF EXISTS (SELECT * FROM information_schema.tables WHERE TABLE_SCHEMA = database() AND table_name LIKE 'object_metadata_%') THEN
                SELECT @obj_meta_dest := CONCAT('DELETE ', table_name, ' FROM ', table_name, ' JOIN delete_obj ON ', table_name, '.dest_id = delete_obj.id;') FROM information_schema.tables WHERE TABLE_SCHEMA = database() AND table_name LIKE 'object_metadata_%';
            ...
            -- delete object_query
            SET @obj_query = CONCAT('DELETE object_query_',classId,' FROM object_query_',classId,' JOIN delete_obj ON delete_obj.id = object_query_',classId,'.oo_id;');
            PREPARE obj_query_stmt FROM @obj_query;
            EXECUTE obj_query_stmt; 
            DEALLOCATE PREPARE obj_query_stmt;
            
            -- object relations destination
            SELECT @obj_relations_dest := CONCAT('DELETE ', table_name, ' FROM ', table_name, ' JOIN delete_obj ON ', table_name, '.dest_id = delete_obj.id AND ', table_name, '.type=''object'';') FROM information_schema.tables WHERE TABLE_SCHEMA = database() AND table_name LIKE 'object_relations_%';
            ...
            SET @obj_relations = CONCAT('DELETE r FROM object_relations_', classId, ' AS r JOIN delete_obj ON delete_obj.id = r.src_id;');
            PREPARE obj_relations_stmt FROM @obj_relations;
            EXECUTE obj_relations_stmt; 
            DEALLOCATE PREPARE obj_relations_stmt;
        
            -- delete object_store  
            SET @obj_store = CONCAT('DELETE s FROM object_store_', classId, ' AS s JOIN delete_obj AS o ON o.id = s.oo_id;');
            ...
            JOIN
                delete_obj
            ON
                object_url_slugs.objectId = delete_obj.id;
            ...
            JOIN
                delete_obj
            ON
                schedule_tasks.cid = delete_obj.id
                AND
            ...
            JOIN
                delete_obj
            ON
                tags_assignment.cid = delete_obj.id
                AND
            ...
            JOIN
                delete_obj
            ON
                users_workspaces_object.cid = delete_obj.id;
